'''
Author: 娄炯
Date: 2021-04-25 13:38:01
LastEditors: loujiong
LastEditTime: 2021-04-26 12:51:30
Description: 
...
def re_scheduling(is_draw=False,
    ...
    edge_weight=(sum([_edge.upload_data_rate for _edge in edge_list])+cloud.data_rate)/(edge_number+1)
    ...
    for _release_index,_release_time in enumerate(release_time_list):
        ...
        for _ap_index,_ta_index in unscheduled_tasks.keys():
            if (_ap_index,_ta_index) not in unscheduled_tasks_sorted_by_remain_length:
                ...
                unscheduled_tasks_sorted_by_remain_length[(_ap_index,_ta_index)] = (can_be_scheduled_label,0-application_list[_ap_index].task_graph.nodes()[_ta_index]["current_remain_length"],application_list[_ap_index].task_graph.nodes()[_ta_index]["current_remain_length"])
...
if __name__ == '__main__':
    is_draw=True
    application_num=2
    application_average_interval=5
    edge_number=5
