...
LastEditTime: 2021-04-19 18:54:00
...
def re_scheduling(is_draw=False,
    ...
    for i in application_list[7].task_graph.nodes():
        print("task: {0}".format(i))
        print(application_list[7].task_graph.nodes()[i])       
    
    for u,v in application_list[7].task_graph.edges():
        print("edge: {0}-{1}".format(u,v))
        print(application_list[7].task_graph.edges()[u,v])

    for i in range(edge_number):
        print(edge_list[i].upload_data_rate)

    print("cloud transmission data rate: {0}".format(cloud.data_rate))  
    draw.draw(application_list[7].task_graph)
